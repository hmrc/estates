...
@Singleton
class GetEstateController @Inject() (
  ...
  def get(utr: String, applyTransforms: Boolean): Action[AnyContent] =
    doGet(utr, applyTransforms) { result: GetEstateResponse =>
  ...
  def getPersonalRepresentative(utr: String): Action[AnyContent] =
    doGet(utr, applyTransforms = true) {
      case processed: GetEstateProcessedResponse =>
        val pick = (JsPath \ Symbol("details") \ Symbol("estate") \ Symbol("entities") \ Symbol(
  ...
  def getDateOfDeath(utr: String): Action[AnyContent] =
    getItemAtPath(
      utr,
      JsPath \ Symbol("details") \ Symbol("estate") \ Symbol("entities") \ Symbol("deceased") \ Symbol("dateOfDeath")
    )

  private def notEnoughInformationResponse(utr: String, json: JsValue, errors: JsValue)(implicit
  ...
  private def getEtmpData(utr: String)(processObject: JsValue => JsValue): Action[AnyContent] =
  ...
  private def doGet(utr: String, applyTransforms: Boolean)(
    handleResult: GetEstateResponse => Result
  ): Action[AnyContent] =

    (validateUTRActionFactory.create(utr) andThen identify).async { implicit request =>
