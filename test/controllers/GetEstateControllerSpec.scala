  ...
  ".get" should {
    ...
    "return 200" when {
      ...
      "estate is processed" in {
        ...
        val etmpJson =
          (getJsonValueFromFile("etmp/playback/valid-estate-playback-01.json") \ "trustOrEstateDisplay").get

        when(mockEstatesService.getEstateInfo(any(), any())(any()))
          .thenReturn(Future.successful(GetEstateProcessedResponse(etmpJson, ResponseHeader("Processed", "1"))))
      ...
      "estate is not processed" in {
        ...
        whenReady(result) { _ =>
          verify(mockAuditService).auditGetVariationFailed(mockEq(utr), any[JsValue])(any(), any())
          status(result)      mustBe OK
          contentType(result) mustBe Some(JSON)
        }
      }
    ...
    "return 400 - BadRequest" when {
      "the UTR given is invalid" in {
        ...
        val result =
          controller.get(invalidUTR, applyTransforms = false).apply(FakeRequest(GET, s"/estates/$invalidUTR"))
    ...
    "return 500 - InternalServerError" when {
      ...
      "response does not contain enough data" in {
        ...
        when(mockEstatesService.getEstateInfo(any(), any())(any()))
          .thenReturn(Future.successful(NotEnoughDataResponse(json, errors)))
  ...
  ".getPersonalRepresentative" should {
    ...
    "return 403 - Forbidden with parked content" in {
      ...
      val result     = controller
        .getPersonalRepresentative(utr)
        .apply(FakeRequest(GET, s"/estates/$utr/transformed/personal-representative"))
    ...
    "return 200 - Ok with processed content" in {
      ...
      when(variationsTransformationService.getTransformedData(any[String], any[String])(any[HeaderCarrier]))
